...
export function JudgeResults({ scores, status, onRetry }: JudgeResultsProps) {
  ...
  if (status === "done" && scores) {
    ...
    const winnerConfig = PERSONA_CONFIG[scores.winner];
      ...
      >
        <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          <div className="rounded-[1.5rem] border border-white/8 bg-white/[0.03] p-5">
            <p className="font-mono text-[0.58rem] uppercase tracking-[0.18em] text-muted-foreground">Winner</p>
            <div className="mt-4 flex items-center gap-3">
              <div className={`h-12 w-12 overflow-hidden rounded-full ${winnerConfig.bgLight}`}>
                <Image src={winnerConfig.avatar} alt={scores.winner} width={48} height={48} className="h-full w-full object-cover" />
              </div>
              <div>
                <p className={`font-display text-3xl ${winnerConfig.color}`}>{scores.winner}</p>
                <p className="font-mono text-[0.55rem] uppercase tracking-[0.18em] text-muted-foreground">Judged winner</p>
              </div>
            </div>
          </div>
          <div className="rounded-[1.5rem] border border-white/8 bg-white/[0.03] p-5">
            <p className="font-mono text-[0.58rem] uppercase tracking-[0.18em] text-muted-foreground">Strongest Moment</p>
            <p className="mt-4 text-sm leading-6 text-foreground/82">{scores.strongestMoment}</p>
          </div>
          <div className="rounded-[1.5rem] border border-white/8 bg-white/[0.03] p-5">
            <p className="font-mono text-[0.58rem] uppercase tracking-[0.18em] text-muted-foreground">Best Exchange</p>
            <p className="mt-4 text-sm leading-6 text-foreground/82">{scores.bestExchange}</p>
          </div>
          <div className="rounded-[1.5rem] border border-white/8 bg-white/[0.03] p-5">
            <p className="font-mono text-[0.58rem] uppercase tracking-[0.18em] text-muted-foreground">Verdict Summary</p>
            <p className="mt-4 text-sm leading-6 text-foreground/82">{scores.summary}</p>
          </div>
        </section>
        ...
        >
          <section className="border-b border-white/8 pb-8 sm:pb-10 lg:border-b-0 lg:border-r lg:pr-8">
            <div className="flex items-start justify-between gap-4 border-b border-white/6 pb-6">
              ...
              <div className="flex h-12 w-12 items-center justify-center overflow-hidden rounded-full bg-yellow-500/10 ring-1 ring-yellow-500/20">
            ...
            <div className="mt-6 space-y-4">
              ...
              <div className="grid gap-4 xl:grid-cols-2">
                <div className="border-b border-white/6 pb-5 xl:border-b-0 xl:border-r xl:pr-5">
                  <p className="font-mono text-[0.6rem] uppercase tracking-[0.18em] text-muted-foreground">Best Exchange</p>
                  <p className="mt-3 text-sm leading-7 text-foreground/78">{scores.bestExchange}</p>
                </div>
              ...
              <div className="border-t border-white/6 pt-5">
                <p className="font-mono text-[0.6rem] uppercase tracking-[0.18em] text-muted-foreground">Standout Lines</p>
                <div className="mt-4 grid gap-3 sm:grid-cols-2">
                  {sortedEvals.map((evaluation) => {
                    const config = PERSONA_CONFIG[evaluation.persona];
                    return (
                      <div key={evaluation.persona} className="rounded-2xl border border-white/8 bg-white/[0.025] p-4">
                        <p className={`font-display text-2xl ${config.color}`}>{evaluation.persona}</p>
                        <p className="mt-2 text-sm leading-6 text-foreground/78">{evaluation.standoutMove}</p>
                      </div>
                    );
                  })}
                </div>
            ...
            <AnimatePresence mode="wait">
              ...
              >
                ...
                <div className="space-y-4 border-b border-white/6 pb-5">
                  <ScoreBar label="Argument Logic" score={currentEval.scores.logic} colorClass={getBgColor(currentEval.persona)} />
                  <ScoreBar label="Clarity" score={currentEval.scores.clarity} colorClass={getBgColor(currentEval.persona)} />
                  <ScoreBar label="Evidence Use" score={currentEval.scores.evidence} colorClass={getBgColor(currentEval.persona)} />
                  <ScoreBar label="Direct Engagement" score={currentEval.scores.engagement} colorClass={getBgColor(currentEval.persona)} />
                </div>
